...
<form role="form" method="post" action="sales_add_new.php">
							...
							?>
                                ...
                                ?>
                                <input type="hidden" class="form-control" name="supplier_id" value="<?php echo $supplier_id;?>" required> 
							...
							<br/>
								...
								<select class="form-control select2" name="pc_id" tabindex="1" autofocus required>
								<?php	
									include 'dist/includes/dbcon.php';								
										$query1=mysqli_query($con,"select * from productcode natural join pdsubcat natural join stproducts where supplier_id='$supplier_id'  ORDER BY pc_id ASC")or die(mysqli_error($con));
										while ($row1=mysqli_fetch_array($query1)){
											$id=$row1['pc_id'];
							?>
										<option value="<?php echo $row1['pc_id'];?>"><?php echo $row1['pc_code'];?> <?php echo $row1['product_name'];?> <?php echo $row1['sc_name'];?></option>
								  <?php }?>
								</select>
                                <span class="fa form-control-feedback right" aria-hidden="true"></span>
						  </div><!-- /.form group --> 
