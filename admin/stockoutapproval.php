						...
						<div class = "x-panel">
						 <table id="datatable" class="table table-striped table-bordered">
							 ...
							 <tbody>
									<?php	
									include 'dbcon.php';								
										$query1=mysqli_query($con,"select * from reviewsales natural join product natural join category natural join reviewsales_details natural join customer ORDER BY reviewsales_id DESC LIMIT 0,1")or die(mysqli_error($con));
										while ($row1=mysqli_fetch_array($query1)){
											$id=$row1['reviewsales_id'];
											
									?>  
								<tr>
									<td><?php echo $row1['reviewsales_id'];?></td>
									<td><?php echo $row1['date_added'];?></td>
									<!--<td><?php echo $row1['expirydate'];?></td>
									<td><?php echo $row1['sales_qty'];?></td>
									<td><?php echo $row1['price'];?></td>-->
									<td><?php echo $row1['cust_last'];?></td>
									...
									<td>
										...
										<?php include 'update_reviewstockout_modal.php';?>
								<?php }?>
							 </tbody>								
						 </table>
						</div>
